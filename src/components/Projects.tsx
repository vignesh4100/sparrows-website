...
const Projects = () => {
  ...
  const handleAddSampleData = async () => {
    try {
      ...
      alert(
        'Failed to add sample data. Please check your Firebase configuration and security rules.'
      );
      setShowSetupMessage(true);
    } finally {
      setLoading(false);
  ...
  const handleAddSampleBlogs = async () => {
    try {
      ...
      alert(
        'Failed to add sample blogs. Please check your Firebase configuration and security rules.'
      );
    } finally {
      setLoading(false);
    }
          ...
          <div className="bg-white rounded-lg shadow-lg p-8 text-center">
            <div className="mb-6">
              ...
              <p className="text-gray-600 mb-6">
                No projects found. Please configure your Firebase Firestore
                security rules to allow data access.
              </p>
            </div>
    ...
    match /projects/{document} {
      ...
      allow write: if true;
            ...
            <p className="text-sm text-gray-500 mt-3">
              Click "Add Sample Blogs Only" to add just the 5 sample blogs, or
              "Add Sample Projects + Blogs" for both
            </p>
          </div>
        </div>
      </section>
                ...
                <div className="absolute top-4 left-4 z-10">
                  <span
                    className={`${getStatusColor(
                      project.status
                    )} text-white px-4 py-2 rounded-full text-sm font-semibold shadow-lg backdrop-blur-sm`}
                  >
                    {project.status}
                  </span>
                </div>
              ...
              <div className="p-8">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.highlights.slice(0, 3).map((feature, index) => (
                    <span
                      key={index}
                      className="bg-gradient-to-r from-red-50 to-red-100 text-red-700 px-3 py-1.5 rounded-full text-xs font-medium border border-red-200"
                    >
                      {feature}
                    </span>
                  ))}
